...
# Function to display probability plot
def probability_plot(model, X, y_test):
    ...
    new_df.round(4)
    new_df['P_High'] = new_df[['Class 0 Prob', 'Class 1 Prob']].max(axis=1)
    new_df['P_Low'] = new_df[['Class 0 Prob', 'Class 1 Prob']].min(axis=1)
    new_df.drop('Class 0 Prob', axis=1, inplace=True)
    new_df.drop('Class 1 Prob', axis=1, inplace=True)
    ...
    new_df['Correct Prediction'] = correct
